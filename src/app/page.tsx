...
function AuthScreen() {
  ...
  const quickLogins = [
    { label: 'Super Admin', email: 'admin@nershield.gov.in' },
    { label: 'State Admin', email: 'state@nershield.gov.in' },
    { label: 'District Officer', email: 'officer@nershield.gov.in' },
    { label: 'Field Officer', email: 'field@nershield.gov.in' },
    { label: 'Logistics Op', email: 'logistics@nershield.gov.in' },
    { label: 'Viewer', email: 'viewer@nershield.gov.in' },
  ];
    ...
    <div className="auth-page">
      <div className="auth-card">
        <div className="auth-logo">
          ...
          <h1>NER-SHIELD AI</h1>
          <p>Predictive Logistics Intelligence Platform</p>
          
          <div style={{ marginTop: '0.625rem', display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '3px 10px', borderRadius: '9999px', background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.25)', fontSize: '0.6875rem', color: '#38bdf8' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: firebaseConnected ? '#10b981' : '#f59e0b', display: 'inline-block' }} />
            <span>Firebase: nerixa-2e6f6 {firebaseConnected ? '(Realtime DB Live)' : '(Cloud Sync Active)'}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          {!isLogin && (
            <>
              ...
              <div className="form-group">
                <label className="form-label">Assigned Command Role</label>
                <select
                  className="form-select"
                  value={role}
                  onChange={e => setRole(e.target.value as UserRole)}
                >
                  <option value="SUPER_ADMIN">Super Admin (Full HQ Access)</option>
                  <option value="STATE_ADMIN">State Admin (State Logistics)</option>
                  <option value="DISTRICT_OFFICER">District Officer (Field Incidents)</option>
                  <option value="FIELD_OFFICER">Field Officer (First Responder)</option>
                  <option value="LOGISTICS_OPERATOR">Logistics Operator (Convoys)</option>
                  <option value="VIEWER">Viewer (Public Monitoring)</option>
                </select>
              </div>
            </>
          )}
          ...
          <div className="form-group">
            ...
            <input
              ...
              placeholder="admin@nershield.gov.in"
              required
            />
          </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '0.75rem' }}>
          ...
          >
            {isLogin ? 'Need an account? Register with Firebase' : 'Already have an account? Sign In'}
          </button>
        </div>

        <div className="auth-divider">Quick Demo Mode Logins</div>
        <div className="auth-quick-login">
          {quickLogins.map(q => (
            <button key={q.email} className="auth-quick-btn" onClick={() => login(q.email, 'demo')}>
              {q.label}
            </button>
          ))}
        </div>
...
function Sidebar({ activePage, onNavigate }: { activePage: string; onNavigate: (page: string) => void }) {
  const { alerts, demoState, language, imageIntelSummary, satelliteSummary } = useApp();
  const criticalAlerts = alerts.filter(a => a.level === 'CRITICAL' && a.status === 'ACTIVE').length;

  const navItems = [
    { id: 'dashboard', icon: DashboardIcon, label: t('nav.dashboard', language) },
    ...
    { id: 'map', icon: MapIcon, label: t('nav.map', language) },
    ...
    { id: 'incidents', icon: HazardIcon, label: t('nav.incidents', language) },
    { id: 'alerts', icon: AlertIcon, label: t('nav.alerts', language), badge: criticalAlerts },
    { id: 'analytics', icon: AnalyticsIcon, label: t('nav.analytics', language) },
  ];
  ...
  return (
    <header className="app-header">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        {/* Satellite Flood Intelligence Demo Button */}
        <button
        ...
        {/* Hackathon Image Scenario Button */}
        <button
          className="btn btn-sm"
          onClick={() => {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: '0.25rem', borderLeft: '1px solid var(--border-color)', paddingLeft: '0.625rem' }}>
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: '0.625rem', color: '#64748b', textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>{user?.role?.replace(/_/g, ' ')}</div>
          </div>
          <button
            className="btn btn-ghost btn-sm"
...
// ── Main App ──
export default function NERShieldApp() {
