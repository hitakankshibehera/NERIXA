...
import type { Road, RouteRequest, SimulationScenario, Incident, Alert, RiskPrediction, RouteOption, SimulationResult, CommanderResponse, IncidentType } from '@/lib/types';
...
function AuthScreen() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      register(email, password, name, 'VIEWER');
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          {!isLogin && (
            <div className="form-group">
              ...
              <input type="text" className="form-input" value={name} onChange={e => setName(e.target.value)} placeholder="Dr. Rajesh Kumar" />
            </div>
          )}
          <div className="form-group">
            <label className="form-label">Email Address</label>
            <input type="email" className="form-input" value={email} onChange={e => setEmail(e.target.value)} placeholder="admin@nershield.gov.in" required />
          </div>
          <div className="form-group">
          ...
          <button type="submit" className="btn btn-primary w-full btn-lg" style={{ marginTop: '0.5rem' }}>
            {isLogin ? 'Sign In to Command Center' : 'Create Account'}
          </button>
        </form>
...
function Header({ 
  ...
  const { user, logout, toggleTheme, theme, language, setLanguage, startDemo, pauseDemo, resetDemo, demoState, liveWeatherReports, triggerHackathonImageScenario, runSatelliteFloodScenario } = useApp();
